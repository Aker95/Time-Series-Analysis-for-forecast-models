...
[EstMdl,~,~] = estimate(MdlBIC,ds.rbrent);
...
for i=0:146
    ...
    [rbrent_pronpuntoo,~] = forecast(Mdl1,1,'Y0',ds.rbrent(1:236+i),'V0',res_AIC,'E0',res_AIC);
...
rng(1234)
[Mdl1,~,~] = estimate(MdlBIC,ds.rbrent(1:236+i)); 
rbrent_Simm = simulate(Mdl1,1,'NumPaths',1000,'Y0',ds.rbrent(1+i:236+i));
...
hold on
plot(ds.FECHA(237:end),IC95_caminata(:,1),'r','LineWidth',.7)
hold on
plot(ds.FECHA(237:end),IC95_caminata(:,2),'r','LineWidth',.7)
ylabel('%')
xlabel('Fecha')
hold off
...
Media_errores_pronpunto=mean(ds.err_rbrent_pronpunto(237:383,1));
...
for i=1:19
    ...
    E.(fname)=zeros (50, 1)
    E.(fname)(ds.rbrent(237:end)<ds.(fname2(237:end))) = 1
