...
plot(datos.FECHA(205:216),For_sua,'k','LineWidth',2)
hold on
plot(datos.FECHA(205:216),var,'b','LineWidth',2)
ylabel('%')
xlabel('Fecha')
legend('Crecimiento ISE', 'Pronóstico Central', 'Varianza ISE')
title('Crecimiento ISE')

%%
[pred,VarPred] = forecast(MdlGARCH1,11,'Y0', datos.est1);

LIC95 = pred - 1.96*sqrt(VarPred);
HIC95 = pred + 1.96*sqrt(VarPred);

%%
y=rmmissing(datos.est1);
For=[y(204);pred];
var_1=[varianzaGarch(204); VarPred];
...
hold on
plot(datos.FECHA(205:216),var_1,'b','LineWidth',2)
ylabel('%')
xlabel('Fecha')
legend('Crecimiento ISE', 'Pronóstico Central', 'Varianza ISE')
title('Crecimiento ISE')